...
`))}o.ok||process.exit(1)}function hs(e,n,t,o){try{let r=Ve(n,e,"",{rootName:t,samplesMode:o});(!r||r.startsWith("// Unsupported"))&&(console.error(_(`typemorph: unsupported format "${e}". Run \`typemorph list\` to see all formats.`)),process.exit(1)),process.stdout.write(r)}catch(r){console.error(_(`typemorph: ${r?.message??String(r)}`)),process.exit(1)}}async function ec(){let e=process.argv.slice(2);if(e.length===0||e.includes("--help")||e.includes("-h")){console.log(Jl);return}if(e.includes("--version")||e.includes("-v")){console.log("0.4.0");return}let n=e[0];if(n==="list"){Wl();return}let t=e.findIndex(m=>m==="--root"||m==="-r"),o=t!==-1?e[t+1]:"Root",r=e.findIndex(m=>m==="--schema"),i=r!==-1?e[r+1]:void 0,s=e.includes("--json");if(n==="reverse"){let m=e.slice(1).find(b=>!b.startsWith("-")),d=m?le(m):await et(),{json:y,error:g}=cs(d);(g||!y)&&(console.error(_(`typemorph reverse: ${g??"No interfaces found"}`)),process.exit(1)),process.stdout.write(y+`
