        ...
        <div className="grid md:grid-cols-2 gap-8">
          {posts.map((post) => (
            <a
              key={post.slug}
              href={`/blog/${post.slug}`}
              className="group dev-card p-8 rounded-[2.5rem] flex flex-col hover:-translate-y-2 transition-all duration-300"
            >
              ...
              <h2 className="text-2xl font-black text-white mb-4 group-hover:text-accent transition-colors leading-tight">
                {post.title}
              </h2>
